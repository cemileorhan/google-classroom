...
namespace Classroom.Controllers
{
    ...
    public class HomeworkController : Controller
    {
        ...
        public IActionResult Index(int? HomeworkId)
        {
            ...
            // Kullanıcı bu ödevde mi (ödevin içinde mi) ve öğrenci mi kontrolü
        ...
        {
            if (HomeworkId == 0 || ClassroomId == 0 || string.IsNullOrEmpty(HwText))
            {
                return BadRequest();
            }

            // Kullanıcının bu sınıfta öğrenci olup olmadığını kontrol et
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var isUserInClassroom = db.ClassUser.Any(cu => cu.ClassRoomId == ClassroomId && cu.ApplicationUserId == userId && !cu.Roles);
            ...
            // Kullanıcının bu ödev için zaten bir kaydı olup olmadığını kontrol et
            if (!db.HomeworkUser.Any(hu => hu.HomeworkId == HomeworkId && hu.ApplicationUserId == userId))
            {
                return Forbid();
            }

            // Ödevin teslim süresinin geçip geçmediğini kontrol et
            var homework = db.Homework.FirstOrDefault(h => h.Id == HomeworkId);
            if (homework == null || homework.DueDate < DateTime.Now)
            {
                return NotFound("Homework not found or due date passed.");
            }
            ...
            return RedirectToAction("Index", new { HomeworkId = HomeworkId });
        }
